...
export default async function Search({params } : { params: {query: string}}) {
    ...
    return (
        <div className="container">
            {params.query === "movie" ? <h1 className="text-4xl font-bold ">Popular Movies</h1> : params.query === "tv" ? <h1 className="text-4xl font-bold ">Popular TV Shows</h1> : params.query === "person" ? <h1 className="text-4xl font-bold ">Popular People</h1> : <h1 className="text-4xl font-bold ">Search Results for "{decodeURI(params.query)}"</h1>}
        </div>
    )
}