...
async function getRecommendations(movieId: number) {
    const response = await fetch(`https://api.themoviedb.org/3/movie/${movieId}/recommendations?api_key=${process.env.TMDB_API_KEY}`)
    const movieRecs = await response.json()
    return movieRecs
}

export default async function Recommendations(movieId: any) {
    const movieRecs = await getRecommendations(movieId.movieId)
